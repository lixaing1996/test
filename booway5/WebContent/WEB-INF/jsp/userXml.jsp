...
<body>
	...
	<form role="search" class="navbar-form navbar-left"
		action="/booway5/userXml" method="get">
			...
			<div class="lt-box">
				<div class="table-responsive">
					<table class="table table-hover">
						...
						<tbody>
							<c:forEach items="${userXml}" var="u" varStatus="status"
								begin="${(pageNosXml-1)*5 }" end="${pageNosXml*5-1}">
								<tr class="${u.id}">
									<td>${u.id}</td>
									<td>${u.name}</td>
									<td>${u.age}</td>
					...
					<center>
						<c:if test="${pageNosXml>1 }">
							<a href="/booway5/page?pageNos=1&flag=3"
								class="btn  btn-primary-alt">首页</a>
							<a href="/booway5/page?pageNos=${pageNosXml-1 }&flag=3"
								class="btn  btn-primary-alt">上一页</a>
						</c:if>
						<c:if test="${pageNosXml <countPageXml }">
							<a href="/booway5/page?pageNos=${pageNosXml+1 }&flag=3"
								class="btn  btn-primary-alt">下一页</a>
							<a href="/booway5/page?pageNos=${countPageXml }&flag=3"
								class="btn btn-primary-alt">末页</a>
						</c:if>
					</center>
					<form action="/booway5/page">
						<h4 align="center">
							共${countPageXml}页 <input type="text" value="${pageNosXml}"
	...
	<script type="text/javascript">
		$("#addUser").click(function() {
			window.location = '/booway5/userXmlAdd?flag=tiaozhuan';
		});

		$("#all").click(function() {
			window.location = '/booway5/userXml';
		});

		function deleteUser() {
			$.ajax({
				type : 'get',
				url : '/booway5/userXmlDelete',
				dataType : 'json',
				data : {
		...
		function updateUser() {
			window.location = "/booway5/userXmlUpdate?id="
